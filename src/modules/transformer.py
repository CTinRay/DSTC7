...
class EncoderLayer(nn.Module):
    def __init__(self, dim_in,
                 n_heads=6, dropout_rate=0.1, dim_ff=1024):
        super(EncoderLayer, self).__init__()
        self.self_attn = MultiHeadedAttention(
            n_heads, dim_in, dropout_rate)
        self.ffn = PositionwiseFeedForward(
            dim_in, dim_ff, dropout_rate)
    ...
    def forward(self, inputs, lengths):
        ...
        # block 1
        ninputs = self.norm1(inputs)
        attn = self.self_attn(ninputs, ninputs, ninputs, mask=mask)
        res_attn = inputs + self.dropout(attn)

        # block 2
        ffn = self.ffn(self.norm2(res_attn))
        res_ffn = res_attn + ffn
        return res_ffn
